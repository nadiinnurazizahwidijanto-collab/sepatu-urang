...
<div class="container-fluid min-vh-100 d-flex flex-column" style="padding-top: 100px; padding-bottom: 100px;">
    ...
    <div class="container d-flex justify-content-between align-items-center mb-4" data-aos="fade-down">
        ...
        <div class="position-relative p-2 bg-white border rounded-circle shadow-sm d-flex align-items-center justify-content-center" style="width: 42px; height: 42px;">
            ...
            @php $badgeCount = \App\Models\Cart::where('user_id', Auth::id())->sum('quantity'); @endphp
            @if($badgeCount > 0)
    ...
    <div class="container-fluid row m-0 justify-content-evenly">
                ...
                <tbody>
                    ...
                    @forelse ($carts as $cart)
                        <tr class="align-middle">
                            ...
                            <td><img src="{{ asset('img/'.$cart->products->image_path) }}" width="60px" class="rounded shadow-sm"></td>
                            <td class="fw-bold text-start small">{{ $cart->products->name }}</td>
                            <td class="small">Rp{{ number_format($cart->products->price, 0, ',', '.') }}</td>
                            <td><span class="badge bg-light text-dark border px-2">{{ $cart->quantity }}</span></td>
                            <td class="fw-bold small">Rp{{ number_format($cart->products->price * $cart->quantity, 0, ',', '.') }}</td>
                        </tr>
        ...
        <div class="col-12 col-lg-4" data-aos="fade-left">
            <form action="{{ route('checkout') }}" method="post" id="checkoutForm" class="shadow-sm p-4 rounded-4 border bg-white">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control bg-light border-0" id="address" name="address" rows="2" placeholder="Tulis alamat lengkap..." required>{{ old('address') }}</textarea>
                </div>
                ...
                <div class="p-3 rounded-3 bg-light mb-4">
                    ...
                    <div class="d-flex justify-content-between text-success mb-1 small">
                        ...
                        <span>- Rp{{ number_format($total * $discountPercent / 100, 0, ',', '.') }}</span>
                    </div>
                    <hr>
                    <div class="d-flex justify-content-between fw-bold fs-5">
                        <span>Total</span>
                        <span id="display_total">Rp{{ number_format($total_after_discount, 0, ',', '.') }}</span>
                    </div>
                </div>
...
<div class="floating-cart-fixed" data-aos="fade-left">
    <a href="{{ route('cart') }}" class="cart-box-fixed">
        <i class="fa-solid fa-cart-shopping fs-5"></i>
        @if($badgeCount > 0)
            <span class="badge rounded-pill">{{ $badgeCount }}</span>
        @endif
    </a>
</div>
...
<script>
    ...
    function prosesWA(address, method, total) {
        ...
        @foreach($carts as $cart) pesan += "• {{ $cart->products->name }} ({{ $cart->quantity }}x)%0A"; @endforeach
